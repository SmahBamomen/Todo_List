          ...
          return ListView.builder(
              ...
              itemBuilder: (BuildContext context, int index) {
                ...
                if( DateFormat('yyyy-MM-dd').format(DateTime.now()) != DateFormat('yyyy-MM-dd').format(DateTime.fromMillisecondsSinceEpoch(
                  ...
                  isUtc: false,
                ).toUtc()).toString() && DateFormat('yyyy-MM-dd').format(DateTime(DateTime.now().year, DateTime.now().month, DateTime.now().day + 1)) != DateFormat('yyyy-MM-dd').format(DateTime.fromMillisecondsSinceEpoch(
                  snapshot.data!.docs[index].data()["date"].millisecondsSinceEpoch,
                  isUtc: false,
                ).toUtc()).toString()){
                  return Dismissible(
